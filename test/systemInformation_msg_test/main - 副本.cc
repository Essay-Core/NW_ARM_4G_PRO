...
//unsigned char data_p[256] = "5040040300010000001C1650902080850000";//数据指针

//sib2 						
//unsigned char data_p[256] = "0000060FD6C8160C002A00200141000029AA55696D0C72944C1C000";//数据指针

//sib3  00043020384000
//unsigned char data_p[256] = "00043020384000";//数据指针

typedef unsigned char uint8;
typedef unsigned int uint32;


//sib1 			0x50,0x40,0x04,0x03,0x00,0x01,0x00,0x00,0x00,0x1C,0x16,0x50,0x90,0x20,0x80,0x85,0x00,0x00
//uint8 data_p[] = {0x50,0x40,0x04,0x03,0x00,0x01,0x00,0x00,0x00,0x1C,0x16,0x50,0x90,0x20,0x80,0x85,0x00,0x00};
//sib1
//7851800125732FD820381B594442041130913E8C050A
//0x78,0x51,0x80,0x01,0x25,0x73,0x2F,0xD8,0x20,0x38,0x1B,0x59,0x44,0x42,0x04,0x11,0x30,0x91,0x3E,0x8C,0x05,0x0A
//70,0x51,0x80,0x01,0x24,0x4E,0xC7,0xA7,0x00,0x18,0x1B,0x5A,0x04,0x42,0x04,0x11,0x30,0x91,0x3E,0x80,0x00,0x00
...
//sib2 00 00 06 0F D6 C8 16 0C 00 2A 00 20 01 41 00 00 29 AA 55 69 6D 0C 72 94 4C 1C 00 0
...
//sib4 0x10,0x08,0x60,0x12,0x00,0x00,0x00
//uint8 data_p[] = {0x10,0x08,0x60,0x12,0x00,0x00,0x00};
 
//sib5 0x00,0x0F,0x54,0x4C,0xB5,0x91,0xA9,0x52,0xD3,0xA2,0x25,0x03,0x08,0x27,0x2D,0x64,0x44,0xA6,0x91,0x04,0xE5,0xAC,0x88,0x98,0x26,0x20,0x9C,0xB5,0x91,0x13,0x1D,0x84,0x13,0x96,0xB2,0x22,0x58,0x00,0x82,0x72,0xD4,0x40,0xA3,0xAB,0x00,0x10,0x03,0x84,0x31,0x5A,0xE9,0x00,0x10,0x03,0xE8,0x31,0x5A,0xD9,0x00,0x10,0x0B,0xB8,0x31,0x5A,0xC9,0x00,0x10,0x0C,0xE4,0x31,0x5A,0xE9,0x00,0x00,0x00
/*
000F544CB591A952D3A2250308272D6444A69104E5AC889826209CB591131D841396B22258008272D440A3AB00100384315AE9001003E8315AD900100BB8315AC900100CE4315AE9000000
*/
...
int PLMN_Identity[6][2];//0用于存储mcc；1用于存储mnc
...
*/	
	...
	for(int i = 0; i< SI_Message_num;i++)
	{
		//0-6
...
//	
	...
	if(option_level3 == 0)
	{
		...
		if(sibTypeAndInfo == 0)
		{
			...
			//printf("option_level8:%d pos:%d, bitSize:%d\n", option_level8, pos, 1 );
			pos+=1;
			int numberOfRA_Preambles =  ExtraData(pos,4);
			printf("numberOfRA_Preambles:%d pos:%d, bitSize:%d\n", numberOfRA_Preambles , pos, 4);
			pos+=4;
			...
			if(option_level8==1)
			{
				...
				printf("sizeOfRA_PreamblesGroupA:%d pos:%d, bitSize:%d\n", sizeOfRA_PreamblesGroupA , pos, 4);
				pos+=4;
				
				//messageSizeGroupA ENUMERATED
				int messageSizeGroupA = ExtraData(pos,2);
				printf("messageSizeGroupA:%d pos:%d, bitSize:%d\n", messageSizeGroupA, pos, 2 );
				pos+=2;
				
				//
				int messagePowerOffsetGroupB = ExtraData(pos,3);
				printf("messagePowerOffsetGroupB:%d pos:%d, bitSize:%d\n", messagePowerOffsetGroupB , pos, 3);
				pos+=3;
			}
			...
			printf("preambleTransMax:%d pos:%d, bitSize:%d\n", preambleTransMax , pos, 4);
			pos+=4;   
			...
			printf("defaultPagingCycle:%d pos:%d, bitSize:%d\n", defaultPagingCycle, pos, 2 );
			pos+=2;
			int nb =ExtraData(pos,3);
			printf("nb:%d pos:%d, bitSize:%d\n", nb, pos, 3);
			pos+=3;
			...
			printf("rootSequenceIndex:%d pos:%d, bitSize:%d INTEGER (0..837)\n", rootSequenceIndex, pos, 10 );
			pos+=10;
			...
			printf("groupAssignmentPUSCH:%d （0。。29）\n", groupAssignmentPUSCH );
			pos+=5;
			...
			if(option_choice==1)
			{
				...
				printf("srs_BandwidthConfig:%d ENUMERATED { bw0, bw1, bw2, bw3, bw4, bw5, bw6, bw7 }\n", srs_BandwidthConfig );
				pos += 3;
				...
				printf("srs_SubframeConfig:%d ENUMERATED { sc0, sc1, sc2, sc3, sc4, sc5, sc6, sc7, sc8, sc9, sc10, sc11, sc12, sc13, sc14, sc15}\n", srs_SubframeConfig );
				pos += 4;
			...
			if(option_level2>1)
			{
				...
				printf("ul_CarrierFreq:%d 0-65535\n", ul_CarrierFreq );
				pos+=16;
			}
			...
			if((option_level1&0x01) == 1)	
			{
				...
				for(int k = 0; k< maxMBSFN_Allocations;k++)
				{
					int  radioframeAllocationPeriod = ExtraData(pos,3);//ENUMERATED {n1, n2, n4, n8, n16, n32},
					printf("radioframeAllocationPeriod:%d ENUMERATED {n1, n2, n4, n8, n16, n32}\n",  radioframeAllocationPeriod);
					pos+=3;
					int  radioframeAllocationOffset =  ExtraData(pos,3);//	INTEGER (0..7),
					printf("radioframeAllocationOffset:%d INTEGER (0..7)\n", radioframeAllocationOffset );
					pos+=3;
			...
			printf("TimeAlignmentTimer:%d {sf500, sf750, sf1280, sf1920, sf2560, sf5120,	sf10240, infinity}\n", TimeAlignmentTimer );
...
int SIB_sib3()
{
	...
	printf("q_Hyst:%d, pos:%d, bitSize:%d\n", q_Hyst, pos, q_Hyst);
	pos+=4;
	...
	if(option ==1)
	{
		...
		printf("t_Evaluation:%d, pos:%d, bitSize:%d\n", t_Evaluation, pos, 3);
		pos+=3;
	    int t_HystNormal= ExtraData(pos,3); 
		printf("t_HystNormal:%d, pos:%d, bitSize:%d\n", t_HystNormal, pos, 3);
		pos+=3;
		int n_CellChangeMedium= ExtraData(pos,4); //INTEGER (1..16),
		printf("n_CellChangeMedium:%d, pos:%d, bitSize:%d\n", n_CellChangeMedium, pos, 4);
		pos+=4;					
		int n_CellChangeHigh= ExtraData(pos,4); //INTEGER (1..16)
		printf("n_CellChangeHigh:%d, pos:%d, bitSize:%d\n", n_CellChangeHigh, pos, 4);
		pos+=4;	
		//q-HystSF
		int sf_Medium = ExtraData(pos,2); //{dB-6, dB-4, dB-2, dB0},
		printf("sf_Medium:%d, pos:%d, bitSize:%d\n", sf_Medium, pos, 2);
		pos+=2;
	    int sf_High= ExtraData(pos,2); 
		printf("sf_High:%d, pos:%d, bitSize:%d\n", sf_High, pos, 2);
		pos+=2;
	}
	...
	if(option ==1)
	{
		...
		printf("s_NonIntraSearch:%d, pos:%d, bitSize:%d\n", s_NonIntraSearch, pos, 5);
		pos+=5;		
	}
	int threshServingLow= ExtraData(pos,5)*2; //INTEGER (0..31) unit:db
	printf("threshServingLow:%d, pos:%d, bitSize:%d\n", threshServingLow, pos, 5);
	pos+=5;				
	
	int cellReselectionPriority	= ExtraData(pos,3); //INTEGER (0..7)
	printf("cellReselectionPriority:%d, pos:%d, bitSize:%d\n", cellReselectionPriority, pos, 3);
	pos+=3;

	//intraFreqCellReselectionInfo
	option = ExtraData(pos,4); 
	pos+=4;
	int q_RxLevMin = ExtraData(pos,6);//INTEGER (-60..-13),
	printf("q_RxLevMin:%d, pos:%d, bitSize:%d\n", q_RxLevMin, pos, 6);
	pos+=6;
	if((option&0x08)>0)
	{
		int p_max =ExtraData(pos,6)-30; //INTEGER (-30..33)
		printf("p_max:%d, pos:%d, bitSize:%d\n", p_max, pos, 6);
		pos+=6;
	}
	if((option&0x04)>0)
	{
		int s_IntraSearch = ExtraData(pos,5)*2; //INTEGER (0..31) unit:db
		printf("s_IntraSearch:%d, pos:%d, bitSize:%d\n", s_IntraSearch, pos, 5);
		pos+=5;				
	}
	if((option&0x02)>0)
	{
		int allowedMeasBandwidth = Bandwidth[ExtraData(pos,3)]; //
		printf("allowedMeasBandwidth:%d, pos:%d, bitSize:%d\n", allowedMeasBandwidth, pos, 3);
		pos+=3;		
		
	}
	int presenceAntennaPort1 = ExtraData(pos,1); //1 or 0
	printf("presenceAntennaPort1:%d, pos:%d, bitSize:%d\n", presenceAntennaPort1, pos, 1);
	pos+=1;		
	int neighCellConfig = ExtraData(pos,2); //BIT STRING (SIZE (2))
	printf("neighCellConfig:%d, pos:%d, bitSize:%d\n", neighCellConfig, pos, 2);
	pos+=2;		
	int t_ReselectionEUTRA=ExtraData(pos,3);//0-7
	printf("t_ReselectionEUTRA:%d, pos:%d, bitSize:%d\n", t_ReselectionEUTRA, pos, 3);
	pos+=3;	
	if((option&0x01)>0)
	{
...
int SIB_sib4()
{
	...
	int intra_cellID[16] ={-1};
	int intra_cellID_black[16][2] ={-1};//下标0存储cellID，1存储范围；
	int intra_cellID_csg[16][2] ={-1};//下标0存储cellID，1存储范围；
	if((option_flag&0x4) >0)
	{
		int maxCellIntra=ExtraData(pos,4);
		pos += 4;
	    for(int i=0; i< maxCellIntra;  i++)
		{  intra_cellID[i] =ExtraData(pos,9);//取值范围INTEGER (0..503)
		   pos += 9;
	...
	if((option_flag&0x02)>0)
	{
		...
		for(int i= 0; i<maxCellIntra; i++)
		{
			...
			pos += 1;
			intra_cellID_black[i][0] = ExtraData(pos,9);//取值范围INTEGER (0..503)
			pos += 9;
			if(cell_potion==1)
			{
				intra_cellID_black[i][1] = ExtraData(pos,4);
	...
	if((option_flag&0x01)>0)
	{
		...
		for(int i= 0; i<maxCellIntra; i++)
		{
			int cell_potion =ExtraData(pos,1);
			pos += 1;
			intra_cellID_csg[i][0] = ExtraData(pos,9);//取值范围INTEGER (0..503)
			pos += 9;
			if(cell_potion==1)
			{
				intra_cellID_csg[i][1] = ExtraData(pos,4);//取值ENUMERATED {n4, n8, n12, n16, n24, n32, n48, n64, n84, n96, n128, n168, n252, n504, spare2, spare1} ，n4赋值4；
				pos += 4;
			}
		}
	}
...
int SIB_sib5()
{	
	int maxFreq = ExtraData(pos,3);//取值范围	INTEGER ::= 8
	pos += 3;
	...
	for(int i = 0;  i< maxFreq;  i++)
	{
	    ...
	    pos += 5;// threshX-Low取值范围INTEGER (0..31)
		...
		pos+=5;//q-OffsetFreq 5bit
		if((option_level&0x02)>0)
		{ 
			int maxCellInter = ExtraData(pos,4);
			pos+=4;
			int cellID_Inter[16];
			for(int j= 0; j<maxCellInter;j++)
			{
				cellID_Inter[j] = ExtraData(pos,9);//取值范围INTEGER (0..503)
				pos+=9;
				pos+=5; //跳过5bit   // Q-OffsetRange
			}
		}
		int InterFreqBlackCell[16] = {-1};
		if((option_level&0x01)>0)
		{
			int maxCellIntra=ExtraData(pos,4);
			pos += 4;
			for(int j= 0;j<maxCellIntra; j++)
			{
				int cell_potion =ExtraData(pos,1);
				pos += 1;
				InterFreqBlackCell[j] = ExtraData(pos,9);//取值范围INTEGER (0..503)
				pos += 9;
				if(cell_potion==1)
				{
//					intra_cellID_csg[i][1] = ExtraData(pos,4);//取值ENUMERATED {n4, n8, n12, n16, n24, n32, n48, n64, n84, n96, n128, n168, n252, n504, spare2, spare1} ，n4赋值4；
	...
	if(option_level1 == 0)
	{
		...
		if(option_level2 == 0)
		{
			...
			if(option_level3 == 0)
			{
				...
				//sib-TypeAndInfo choice
				int sibTypeAndInfo =  ExtraData(pos,6)+2;
...
{	
	...
	if(option_level3 == 0)
	{
		...
		//int option_level5 =  ExtraData(pos,1);
		//pos +=1;
