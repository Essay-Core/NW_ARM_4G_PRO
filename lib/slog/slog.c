...
#define TIME_STR_FMT                "%04d%02d%02d_%02d%02d%02d"
...
int init_logger(const char *log_dir, slog_level level)
{
    ...
    //snprintf(log_filepath, sizeof(log_filepath) - 1, "%s/%s.log", log_dir, timestr);
    snprintf(log_filepath, 64, "%s/%s.log", log_dir, timestr);
